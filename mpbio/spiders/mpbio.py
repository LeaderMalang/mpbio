...
class MpbioSpider(scrapy.Spider):
    ...
    def parse_single_product(self,response):
        ...
        descript_table=response.css('#product-attribute-specs-table tbody tr td::text').getall()

        items.update({
            'name': name if name is not None else "Not Found",
            'sku':sku if sku is not None else "Not Found",
            'cas_number': cas_number if cas_number is not None else "Not Found",
            'molecular_formula': molecular_formula if molecular_formula is not None else "Not Found",
            'descript_table': descript_table if descript_table is not None else "Not Found",
            'categories':categories if categories is not None else "Not Found"
        })
